...
export const getLicenseSummary = async ({
  ...
  type,
  contract,
}: CI360RequiredProps & { contract?: number }) => {
  const { data } = await ci360AxiosClient.get<LicenseSummary>(
    `/account/license/summary/${accountId}`,
    {
      params: {
        type,
        contract,
...
export const getHealthSummary = async ({
  ...
  const { data } = await ci360AxiosClient.get<HealthSummary>(
    `/account/health/summary/${accountId}`,
    {
      params: {
        type,
      },
...
export const getAccountOverviewSummary = async ({
  ...
  const { data } = await ci360AxiosClient.get<AccountOverviewSummary>(
    `/account/overview/summary/${accountId}`,
    {
      params: {
        type,
      },
