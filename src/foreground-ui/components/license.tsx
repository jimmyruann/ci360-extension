...
export const License = (props: LicenseProps) => {
	const { data: licenseSummary, isLoading, isError, isIdle } = useQuery(["getLicenseSummary", props], () => getLicenseSummary(props));
	...
	return (
		<Box>
			{licenseSummary.measurements.map((measurement, i) => {
				return (
					<div key={i}>
						<b>{measurement.title}:</b> {measurement.quota.value}/{measurement.quota.limit}
					</div>
				);
			})}
		</Box>
