...
export const EnvForm = ({ handleSubmit }: EnvFormProps) => {
  const isAtAccountPage = new RegExp(/\/account\/(saas|managed)\/\d+/).test(
  ...
  if (isAtAccountPage) {
    const [_, type, accountId] = window.location.pathname
      .substring(1)
      .split("/");
    initialValues = {
      type,
        ...
        <Select
          ...
          description={
            <>
              ...
              https://ci360.dynatrace.com/account/<b>{`<env-type>`}</b>/1234
            </>
          }
